...
export class DashboardViewProvider implements vscode.WebviewViewProvider {
  ...
  constructor(
    ...
    // Listen for task changes and update the webview
  ...
  private updateWebview(): void {
    ...
    this._view.webview.postMessage({
      ...
      tasks: tasks.map((t) => ({
        ...
        outputTail: t.output.slice(-20).join(''),
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .task-output {
    margin-top: 6px; padding: 6px; font-family: var(--vscode-editor-font-family);
    font-size: 11px; background: var(--bg); border-radius: 3px;
...
<div class="section">
  ...
  <div class="config-info">
    Platform: <code>${config.platform}</code> &nbsp;|&nbsp;
    Repos: <code>${config.repos.length}</code>
  </div>
</div>
...
<div class="section">
  ...
  <div class="input-group">
    <input type="text" id="issueUrl" placeholder="Issue URL or description..." />
...
function killTask(taskId) {
  vscode.postMessage({ type: 'killTask', taskId });
}

function cleanupTask(taskId) {
  vscode.postMessage({ type: 'cleanupTask', taskId });
}

function openWorktree(taskId) {
  vscode.postMessage({ type: 'openWorktree', taskId });
}
...
function renderTasks(tasks) {
  ...
  container.innerHTML = tasks.map(t => \`
    <div class="task-card">
      ...
      <div class="task-meta">
        \${t.type} &nbsp;|&nbsp; Started \${formatTime(t.createdAt)}
      ...
      \${t.outputTail ? '<div class="task-output">' + escapeHtml(t.outputTail) + '</div>' : ''}
    </div>
  \`).join('');
}
